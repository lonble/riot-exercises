...
static ssize_t _led_handler(coap_pkt_t *pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx) {
    ...
    ssize_t resp_len = 0;
    int led_status = 0;
    coap_method_t method = coap_get_method(pdu);

    switch (method) {
    case COAP_METHOD_PUT: /* on PUT, we set the status of the LED based on the payload */
    ...
    case COAP_METHOD_GET: /* on GET, we return the status of the LED in plain text */
