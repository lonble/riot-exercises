...
4. 板子的输出有缓存，大小为**128 字节**，空间不足时旧数据会被清除，遵循FIFO
5. `pyterm`从板子的缓存中消费数据，也遵循FIFO，被消费的数据会从缓存中消失
6. 板子Reset会清空缓存

`1.`说明运行`make term`时程序可能已经结束了。`5.`说明就算程序结束了，也能看到输出，但只能看一次。`4.`说明如果程序输出较多，不及时连接`pyterm`会导致丢失数据
